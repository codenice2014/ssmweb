	...
	<div class="addEdit-block" :style='{"padding":"10px 30px 30px","background":"url(http://codegen.caihongy.cn/20230228/47752d6be3a94e8892a83ec2a70c8443.png)"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="代取时间" prop="daiqushijian">
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.daiqushijian" 
						type="datetime"
						:readonly="ro.daiqushijian"
						placeholder="代取时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.daiqushijian" label="代取时间" prop="daiqushijian">
					<el-input v-model="ruleForm.daiqushijian" placeholder="代取时间" readonly></el-input>
				</el-form-item>
...
export default {
	...
	created() {
		this.ruleForm.daiqushijian = this.getCurDateTime()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='daiqushijian'){
							this.ruleForm.daiqushijian = obj[o];
							this.ro.daiqushijian = true;
							continue;
						}
						if(o=='zhenduanbaogao'){
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `daiqubaogao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.daiqubaogaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
