	...
	<div class="addEdit-block" :style='{"padding":"10px 30px 30px","background":"url(http://codegen.caihongy.cn/20230228/47752d6be3a94e8892a83ec2a70c8443.png)"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="挂号费" prop="guahaofei">
					<el-input v-model="ruleForm.guahaofei" placeholder="挂号费" clearable  :readonly="ro.guahaofei"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="挂号费" prop="guahaofei">
					<el-input v-model="ruleForm.guahaofei" placeholder="挂号费" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="号码" prop="haoma">
					<el-input v-model="ruleForm.haoma" placeholder="号码" clearable  :readonly="ro.haoma"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="号码" prop="haoma">
					<el-input v-model="ruleForm.haoma" placeholder="号码" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="挂号备注" prop="guahaobeizhu">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.guahaobeizhu" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.guahaobeizhu" label="挂号备注" prop="guahaobeizhu">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.guahaobeizhu"></span>
                </el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='guahaobeizhu'){
							this.ruleForm.guahaobeizhu = obj[o];
							this.ro.guahaobeizhu = true;
							continue;
						}
						if(o=='zhanghao'){
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.guahaobeizhu = this.ruleForm.guahaobeizhu.replace(reg,'../../../ssmgo829/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `wenzhenpaidui/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.wenzhenpaiduiCrossAddOrUpdateFlag = false;
						 this.parent.search();
